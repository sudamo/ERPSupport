...
namespace ERPSupport.SQL.K3Cloud
{
    ...
    public static class PrdAllocation
    {
        ...
        public static DataTable GetTransPZ(string pFNeedDate, int pStockID, int pDeptID)//盆子仓
        {
            ...
            _SQL += string.Format(" , AC.FLOT 批号,MOE.FQTY 调拨数量,MOE.FPRODUCTIONSEQ 生产顺序号,TO_CHAR(MOE.F_PAEZ_DESCRIPTION) 生产订单备注,NVL(MTLSK.F_PAEZ_FMinsendQty,0) 最小批量,MTL.F_PAEZ_SENDPERCENT 发料百分比 ");
            _SQL += string.Format(" , NVL(ASS.FNUMBER,' ') 调拨类型,NVL(ASSL2.FDATAVALUE,' ') 启用批次,NVL(ASSL3.FDATAVALUE,' ') 启用序列号,NVL(ASSL4.FDATAVALUE,' ') 发货类别,CASE WHEN A.FUNITID = 101045 THEN CEIL(MOE.FQTY/{0}) ELSE A.FQTY END 合并数量,CASE WHEN MOE.FHeadDeliveryWay = '58709fcc96a192' AND MOE.FQTY >= 10 THEN 1 ELSE 0 END 大单 ", GlobalParameter.Dir_DPQtyPZ);
            ...
            _SQL += string.Format(" INNER JOIN T_PRD_MOENTRY_A MOA ON MOE.FENTRYID = MOA.FENTRYID AND MOA.FSTATUS IN(3,4) ");
            _SQL += string.Format(" INNER JOIN T_BD_MATERIAL MTL ON AE.FMATERIALID = MTL.FMATERIALID AND MTL.FUSEORGID = 100508 ");
            ...
            _SQL += string.Format(" INNER JOIN T_AUTO_MSTOCKSETTING MST ON AE.FMATERIALID = MST.FMATERIALID AND DEP.FDEPTID = MST.FDEPTID ");
        ...
        public static DataTable GetTransCL(string pFNeedDate, int pStockID, int pDeptID, string pCondition)
        {
            ...
            _SQL += string.Format(" ,AE.FUNITID 单位,UNT.FNUMBER 单位编码,UNTL.FNAME 单位名称,MOE.FWORKSHOPID 领料部门,DEP.FNUMBER 领料部门编码,DEPL.FNAME 领料部门名称 ");
            _SQL += string.Format(" ,AC.FSUPPLYORG 调入库存组织,NVL(ORG2.FNUMBER,' ') 调入库存组织编码,NVL(ORGL2.FNAME,' ') 调入库存组织名称,DEP.FINSTOCKID 调入仓库,STK2.FNUMBER 调入仓库编码,STKL2.FNAME 调入仓库名称,STK2.FDEFSTOCKSTATUSID 调入库存状态,STT2.FNUMBER 调入库存状态编码,STTL2.FNAME 调入库存状态名称 ");
            _SQL += string.Format(" ,AC.FSRCTRANSORGID 调出库存组织,NVL(ORG3.FNUMBER,' ') 调出库存组织编码,NVL(ORGL3.FNAME,' ') 调出库存组织名称,MST.FSTOCKID 中间仓库,STK3.FNUMBER 中间仓库编码,STKL3.FNAME 中间仓库名称,STK3.FDEFSTOCKSTATUSID 调出库存状态,STT3.FNUMBER 调出库存状态编码,STTL3.FNAME 调出库存状态名称 ");
        ...
        public static DataTable GetTransCL(string pFNeedDate, int pStockID, int pDeptID, string pCondition, bool pIsTran, int pOrgId = 100508)//根据仓库属性-是否汇总 判断汇总信息。
        {
            if (pIsTran)
            {
                ...
                _SQL += string.Format(" ,AC.FSUPPLYORG 调入库存组织,NVL(ORG2.FNUMBER,' ') 调入库存组织编码,NVL(ORGL2.FNAME,' ') 调入库存组织名称,DEP.FINSTOCKID 调入仓库,STK2.FNUMBER 调入仓库编码,STKL2.FNAME 调入仓库名称,STK2.FDEFSTOCKSTATUSID 调入库存状态,STT2.FNUMBER 调入库存状态编码,STTL2.FNAME 调入库存状态名称 ");
                _SQL += string.Format(" ,AC.FSRCTRANSORGID 调出库存组织,NVL(ORG3.FNUMBER,' ') 调出库存组织编码,NVL(ORGL3.FNAME,' ') 调出库存组织名称,MST.FSTOCKID 调出仓库,STK3.FNUMBER 调出仓库编码,STKL3.FNAME 调出仓库名称,STK3.FDEFSTOCKSTATUSID 调出库存状态,STT3.FNUMBER 调出库存状态编码,STTL3.FNAME 调出库存状态名称 ");
                _SQL += string.Format(" ,MST.FTRANSTOCKID 中间仓库,STK4.FNUMBER 中间仓库编码,STKL4.FNAME 中间仓库名称,STK4.FDEFSTOCKSTATUSID 中间库存状态,STT4.FNUMBER 中间库存状态编码,STTL4.FNAME 中间库存状态名称 ");
                _SQL += string.Format(" ,AC.FLOT 批号,SUM(AE.FMUSTQTY) 调拨数量,N' ' 生产顺序号,' ' 生产订单备注,NVL(MTLSK.F_PAEZ_FMinsendQty,0) 最小批量,MTL.F_PAEZ_SENDPERCENT 发料百分比 ");
                _SQL += string.Format(" ,NVL(ASS.FNUMBER,' ') 调拨类型,NVL(ASSL2.FDATAVALUE,' ') 启用批次,NVL(ASSL3.FDATAVALUE,' ') 启用序列号,N' ' 发货类别 ");
                _SQL += string.Format(" FROM T_PRD_PPBOM A ");
                _SQL += string.Format(" INNER JOIN T_PRD_PPBOMENTRY AE ON A.FID = AE.FID AND AE.F_PAEZ_PICKTOWMS = 0 AND AE.FPAEZHAVEDIRECT = 0 ");
                ...
                _SQL += string.Format(" INNER JOIN T_BD_STOCKSTATUS STT3 ON STK3.FDEFSTOCKSTATUSID = STT3.FSTOCKSTATUSID ");
                _SQL += string.Format(" INNER JOIN T_BD_STOCKSTATUS_L STTL3 ON STT3.FSTOCKSTATUSID = STTL3.FSTOCKSTATUSID AND STTL3.FLOCALEID = 2052 ");
                _SQL += string.Format(" INNER JOIN T_BD_STOCK STK4 ON MST.FTRANSTOCKID = STK4.FSTOCKID ");
                _SQL += string.Format(" INNER JOIN T_BD_STOCK_L STKL4 ON STK4.FSTOCKID = STKL4.FSTOCKID AND STKL4.FLOCALEID = 2052 ");
                _SQL += string.Format(" INNER JOIN T_BD_STOCKSTATUS STT4 ON STK4.FDEFSTOCKSTATUSID = STT4.FSTOCKSTATUSID ");
                _SQL += string.Format(" INNER JOIN T_BD_STOCKSTATUS_L STTL4 ON STT4.FSTOCKSTATUSID = STTL4.FSTOCKSTATUSID AND STTL3.FLOCALEID = 2052 ");
        ...
        public static void UpdateDirFieldsCL(string pFNeedDate, int pOrgId = 100508)
        {
            ...
            _SQL += string.Format(" WHERE FPAEZHAVEDIRECT = 0 AND F_PAEZ_PICKTOWMS = 0 AND FENTRYID IN( ");
            _SQL += string.Format(" SELECT A.FENTRYID ");
            _SQL += string.Format(" FROM T_PRD_PPBOMENTRY A ");
            _SQL += string.Format(" INNER JOIN T_PRD_PPBOM E ON A.FID = E.FID AND E.FDOCUMENTSTATUS = 'C' ");
            _SQL += string.Format(" INNER JOIN T_PRD_MOENTRY G ON A.FMOENTRYID = G.FENTRYID AND TO_CHAR(G.FPLANSTARTDATE,'yyyy-mm-dd') = TO_CHAR(A.FNEEDDATE,'yyyy-mm-dd') ");
            _SQL += string.Format(" INNER JOIN T_PRD_MO H ON G.FID = H.FID AND H.FDOCUMENTSTATUS = 'C' ");
            ...
            _SQL += string.Format(" INNER JOIN T_BD_DEPARTMENT L ON G.FWORKSHOPID = L.FDEPTID ");
            _SQL += string.Format(" INNER JOIN T_BD_STOCK M ON L.FINSTOCKID = M.FSTOCKID ");
            _SQL += string.Format(" INNER JOIN T_AUTO_MSTOCKSETTING N ON A.FMATERIALID = N.FMATERIALID AND L.FDEPTID = N.FDEPTID ");
            _SQL += string.Format(" INNER JOIN T_BD_STOCK O ON N.FSTOCKID = O.FSTOCKID ");
        ...
        public static int Asyn_PPBom_FNeedDate(DateTime pFrom, DateTime pTo)
        {
            ...
            _SQL += string.Format(" FROM T_PRD_PPBOM A ");
            _SQL += string.Format(" INNER JOIN T_PRD_PPBOMENTRY AE ON A.FID = AE.FID ");
            _SQL += string.Format(" LEFT JOIN T_PRD_MOENTRY BE ON AE.FMOENTRYID = BE.FENTRYID ");
            _SQL += string.Format(" LEFT JOIN T_PRD_MO B ON BE.FID = B.FID ");
        ...
        public static void Syn_PPBom_FNeedDate(DateTime pFrom, DateTime pTo)
        {
            ...
            _SQL += string.Format(" WHERE EXISTS( ");
            _SQL += string.Format(" SELECT 1  ");
            _SQL += string.Format(" FROM T_PRD_MOENTRY BE WHERE AE.FMOENTRYID = BE.FENTRYID AND TO_CHAR(AE.FNEEDDATE,'yyyy-mm-dd') <> TO_CHAR(BE.FPLANSTARTDATE,'yyyy-mm-dd') AND BE.FPLANSTARTDATE BETWEEN TO_DATE('{0}','yyyy-mm-dd') AND TO_DATE('{1}','yyyy-mm-dd')) ", pFrom.ToString("yyyy-MM-dd"), pTo.ToString("yyyy-MM-dd"));
            ORAHelper.ExecuteNonQuery(_SQL);
        }
        ...
        public static void UpdateMST_Tran()
        {
            ...
            _SQL += string.Format(" UPDATE T_AUTO_MSTOCKSETTING SET FTRANSTOCKID = 897782,FMODIFYDATE = SYSDATE WHERE FSTOCKID = 467795671 AND FTRANSTOCKID = 0 AND FDEPTID IN(100684,100685,100686,100687,100688,100707,465531838,467909049); ");
            _SQL += string.Format(" END;");
            ORAHelper.ExecuteNonQuery(_SQL);
        }
