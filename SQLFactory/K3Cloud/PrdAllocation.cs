    ...
    public static class PrdAllocation
    {
        ...
        public static DataTable GetTransCL(string pFNeedDate, int pStockID, int pDeptID, string pCondition)
        {
            _SQL = @"SELECT AC.FOWNERID 货主,NVL(ORG.FNUMBER, 'HN02') 货主编码,NVL(ORGL.FNAME, '河南工厂') 货主名称, AC.FOWNERTYPEID 货主类型,AE.FMATERIALID 物料,MTL.FNUMBER 物料编码,MTLL.FNAME 物料名称
                  ,AE.FUNITID 单位,UNT.FNUMBER 单位编码,UNTL.FNAME 单位名称,MOE.FWORKSHOPID 领料部门,DEP.FNUMBER 领料部门编码,DEPL.FNAME 领料部门名称
                  ,AC.FSUPPLYORG 调入库存组织,NVL(ORG2.FNUMBER,' ') 调入库存组织编码,NVL(ORGL2.FNAME,' ') 调入库存组织名称,DEP.FINSTOCKID 调入仓库,STK2.FNUMBER 调入仓库编码,STKL2.FNAME 调入仓库名称,STK2.FDEFSTOCKSTATUSID 调入库存状态,STT2.FNUMBER 调入库存状态编码,STTL2.FNAME 调入库存状态名称
                  ,AC.FSRCTRANSORGID 调出库存组织,NVL(ORG3.FNUMBER,' ') 调出库存组织编码,NVL(ORGL3.FNAME,' ') 调出库存组织名称,MST.FSTOCKID 中间仓库,STK3.FNUMBER 中间仓库编码,STKL3.FNAME 中间仓库名称,STK3.FDEFSTOCKSTATUSID 调出库存状态,STT3.FNUMBER 调出库存状态编码,STTL3.FNAME 调出库存状态名称
        ...
        public static DataTable GetTransCL(string pFNeedDate, int pStockID, int pDeptID, string pCondition, bool pIsTran, int pOrgId = 100508)//根据仓库属性-是否汇总 判断汇总信息。
        {
            if (pIsTran)
            {
                ...
                INNER JOIN T_PRD_MO MO ON MO.FID = MOE.FID AND MO.FDOCUMENTSTATUS = 'C'
                INNER JOIN T_PRD_MOENTRY_A MOA ON MOE.FENTRYID = MOA.FENTRYID AND MOA.FSTATUS IN(3,4)
                INNER JOIN T_BD_MATERIAL MTL ON AE.FMATERIALID = MTL.FMATERIALID AND MTL.FUSEORGID = {0}
                INNER JOIN T_BD_MATERIAL_L MTLL ON MTL.FMATERIALID = MTLL.FMATERIALID AND MTLL.FLOCALEID = 2052
                LEFT JOIN T_BD_MATERIALSTOCK MTLSK ON MTL.FMATERIALID = MTLSK.FMATERIALID
                INNER JOIN T_BD_UNIT UNT ON AE.FUNITID = UNT.FUNITID
                INNER JOIN T_BD_UNIT_L UNTL ON UNT.FUNITID = UNTL.FUNITID AND UNTL.FLOCALEID = 2052
                ...
                INNER JOIN T_BD_STOCKSTATUS_L STTL4 ON STT4.FSTOCKSTATUSID = STTL4.FSTOCKSTATUSID AND STTL3.FLOCALEID = 2052 ", pOrgId);
            ...
            else
            {
                ...
                INNER JOIN T_PRD_MOENTRY MOE ON AE.FMOENTRYID = MOE.FENTRYID AND TO_CHAR(MOE.FPLANSTARTDATE,'yyyy-mm-dd') = TO_CHAR(AE.FNEEDDATE,'yyyy-mm-dd')
                INNER JOIN T_PRD_MO MO ON MO.FID = MOE.FID AND MO.FDOCUMENTSTATUS = 'C'
                INNER JOIN T_PRD_MOENTRY_A MOA ON MOE.FENTRYID = MOA.FENTRYID AND MOA.FSTATUS IN(4)
                INNER JOIN T_BD_MATERIAL MTL ON AE.FMATERIALID = MTL.FMATERIALID AND MTL.FUSEORGID = {0}
                INNER JOIN T_BD_MATERIAL_L MTLL ON MTL.FMATERIALID = MTLL.FMATERIALID AND MTLL.FLOCALEID = 2052
                LEFT JOIN T_BD_MATERIALSTOCK MTLSK ON MTL.FMATERIALID = MTLSK.FMATERIALID
                INNER JOIN T_BD_UNIT UNT ON AE.FUNITID = UNT.FUNITID
                ...
                INNER JOIN T_BD_STOCK_L STKL3 ON STK3.FSTOCKID = STKL3.FSTOCKID AND STKL3.FLOCALEID = 2052
                INNER JOIN T_BD_STOCKSTATUS STT3 ON STK3.FDEFSTOCKSTATUSID = STT3.FSTOCKSTATUSID
                INNER JOIN T_BD_STOCKSTATUS_L STTL3 ON STT3.FSTOCKSTATUSID = STTL3.FSTOCKSTATUSID AND STTL3.FLOCALEID = 2052 ", pOrgId);
        ...
        public static void UpdateDirFieldsCL(string pFNeedDate, int pOrgId = 100508)
        {
            ...
            if (pOrgId == 100508)
                _SQL += " INNER JOIN T_PRD_MOENTRY_A I ON A.FMOENTRYID = I.FENTRYID AND I.FSTATUS IN(3,4) ";
            else
                _SQL += " INNER JOIN T_PRD_MOENTRY_A I ON A.FMOENTRYID = I.FENTRYID AND I.FSTATUS IN(4) ";
        ...
        public static bool SetDefaultStock(DateTime pDateTime, Model.Enum.FormID pFormId)
        {
            ...
            if (pFormId == Model.Enum.FormID.PRD_PPBOM)
                ...
                _SQL += string.Format(" INNER JOIN T_PRD_MOENTRY_A AA ON AE.FENTRYID = AA.FENTRYID AND AA.FSTATUS IN(4) ");
        ...
        {
            //_SQL = @"SELECT 0 CHB,A.FDATE 日期,A.FBILLNO 单据编号,CASE A.FDOCUMENTSTATUS WHEN 'A' THEN '创建' WHEN 'B' THEN '审核中' WHEN 'C' THEN '已审核' WHEN 'D' THEN '重新审核' WHEN 'Z' THEN '暂存' END 数据状态,CUSTL.FNAME 客户,A.F_CONTACTS 联系人
            //FROM T_SAL_DELIVERYNOTICE A
            //INNER JOIN T_SAL_DELIVERYNOTICEENTRY AE ON A.FID = AE.FID
            ...
            //    _SQL += " WHERE TO_CHAR(A.FDATE,'YYYY-MM-DD') = '" + pFDate.ToString("yyyy-MM-dd") + "'";
            //else
            //    _SQL += " WHERE A.FBILLNO = '" + pFBillNo + "'";
            //_SQL += " GROUP BY A.FDATE,A.FBILLNO,A.FDOCUMENTSTATUS,CUSTL.FNAME,A.F_CONTACTS HAVING SUM(CASE WHEN AE.FBASETRANSFERQTY > 0 THEN 1 ELSE 0 END) = 0 ORDER BY A.FBILLNO";
