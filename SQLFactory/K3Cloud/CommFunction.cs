...
namespace ERPSupport.SQL.K3Cloud
{
    ...
    public class CommFunction : ICommFunction
    {
        ...
        /// <returns></returns>
        public string ConnectionCheck_ORA(string pConnectionString)
        {
        ...
        /// <param name="pConnectionString">数据库连接字符串</param>
        /// <returns></returns>
        public string ConnectionCheck_SQL(string pConnectionString)
        {
        ...
        public int GetUserId(string pConnectionString, string pUserName)
        {
            _SQL = string.Format("SELECT FUSERID FROM T_SEC_USER WHERE FNAME = '{0}'", pUserName);
        ...
        /// <returns></returns>
        public int GetDepartIdByNumber(int pFUseOrgId, string pFNumber)
        {
            _SQL = string.Format("SELECT FDEPTID FROM T_BD_DEPARTMENT WHERE FUSEORGID = {0} AND FNUMBER = '{1}'", pFUseOrgId, pFNumber);
        ...
        /// <returns></returns>
        public DataTable GetDepartment(int pType, int pUseOrgId, string pProperty)
        {
            switch (pType)
            {
                case 1:
        ...
        /// </summary>
        /// <returns></returns>
        public List<string> GetPickMtlDepartment(int pUseOrgId = 100508)
        {
            List<string> lstReturn = new List<string>();
            _SQL = string.Format("SELECT FNUMBER FROM DM_PICKMTLDEPARTMENT WHERE ISDELETE = '0' AND FUSEORGID = {0} ORDER BY FNUMBER", pUseOrgId);
            DataTable dt = ORAHelper.ExecuteTable(_SQL);
        ...
        /// <returns></returns>
        public int GetCustomerId(string pCustomerName, int pUseOrgId)
        {
        ...
        /// <returns></returns>
        public DataTable GetCustomerListByName(string pCostomerName)
        {
        ...
        /// </summary>
        /// <param name="pUseOrgId">使用组织</param>
        /// <param name="pType">FVALUE类型</param>
        /// <returns></returns>
        ...
        /// <param name="pFNumber">编码</param>
        /// <returns></returns>
        public int GetStockIdByNumber(int pFUseOrgId, string pFNumber)
        {
            _SQL = string.Format("SELECT FSTOCKID FROM T_BD_STOCK WHERE FUSEORGID = {0} AND FNUMBER = '{1}'", pFUseOrgId, pFNumber);
            _obj = ORAHelper.ExecuteScalar(_SQL);
        ...
        /// <returns></returns>
        public DataTable GetStock(int pType = -1, int pUserOrgId = 100508)
        {
            switch (pType)
            {
                case 1:
                    ...
                    break;
                case 2:
                    _SQL = string.Format("SELECT SK.FNUMBER||'|'||SK.FSTOCKID FVALUE,SKL.FNAME ");
                    _SQL += string.Format(" FROM T_BD_STOCK SK ");
                    _SQL += string.Format(" INNER JOIN T_BD_STOCK_L SKL ON SK.FSTOCKID = SKL.FSTOCKID AND SKL.FLOCALEID = 2052 ");
                    _SQL += string.Format(" WHERE SK.FDOCUMENTSTATUS = 'C' AND SK.FFORBIDSTATUS = 'A' AND SK.FUSEORGID = {0} ORDER BY SK.FNUMBER ", pUserOrgId);
                    break;
                case 3:
                    _SQL = string.Format("SELECT N'-1' FVALUE,N'请选择' FNAME FROM DUAL ");
                    _SQL += string.Format(" UNION ALL ");
                    _SQL += string.Format(" SELECT SK.FNUMBER||'|'||SK.FSTOCKID FVALUE,SKL.FNAME ");
                    _SQL += string.Format(" FROM T_BD_STOCK SK ");
                    _SQL += string.Format(" INNER JOIN T_BD_STOCK_L SKL ON SK.FSTOCKID = SKL.FSTOCKID AND SKL.FLOCALEID = 2052 ");
                    _SQL += string.Format(" WHERE SK.FDOCUMENTSTATUS = 'C' AND SK.FFORBIDSTATUS = 'A' AND SK.FDEFSTOCKSTATUSID = 10000 AND SK.FUSEORGID = {0} ORDER BY FVALUE ", pUserOrgId);
                    break;
                case 4:
                    ...
                    break;
                case 5:
                    _SQL = string.Format("SELECT SK.FNUMBER FVALUE,SKL.FNAME ");
                    _SQL += string.Format(" FROM T_BD_STOCK SK ");
                    _SQL += string.Format(" INNER JOIN T_BD_STOCK_L SKL ON SK.FSTOCKID = SKL.FSTOCKID AND SKL.FLOCALEID = 2052 ");
                    //_SQL += string.Format(" INNER JOIN T_BD_STOCKGROUP SKG ON SK.FGROUP = SKG.FID ");
                    _SQL += string.Format(" WHERE SK.FDOCUMENTSTATUS = 'C' AND SK.FFORBIDSTATUS = 'A' AND SK.FUSEORGID = {0} AND SKL.FNAME NOT LIKE '%VMI%' ORDER BY SK.FNUMBER", pUserOrgId);
                    break;
                default:
        ...
        public DataTable GetOrganization(int pAcctype = 2, bool pIsContains = false)
        {
            ...
            if (pIsContains)
                ...
                _SQL += string.Format(" WHERE ORG.FDOCUMENTSTATUS = 'C' AND ORG.FFORBIDSTATUS = 'A' AND ORG.FACCTORGTYPE = {0} ORDER BY ORGL.FNAME ", pAcctype);
        ...
        /// <returns></returns>
        public DataTable GetOrganization(string pType)
        {
        ...
        public int ApplicationFlag(string pConnectionString, string pAppName)
        {
            _SQL = string.Format("SELECT FLAG FROM DM_APP_MANAGE WHERE ISDEL = '0' AND APPNAME = '{0}'", pAppName);
        ...
        /// <returns></returns>
        public DataTable GetNavigation(out string pRIDs, out string pMIDs, out string pFunctionIds)
        {
            //获取二级菜单NODEID
            DataTable dtRuleS;
            object oTemp;
            ...
            _SQL = string.Format("SELECT RIDS FROM DM_USER_ROLE WHERE USERID = {0}", GlobalParameter.K3Inf.UserId);
            oTemp = ORAHelper.ExecuteScalar(_SQL);

            if (oTemp != null && oTemp.ToString().Trim() != string.Empty)
            {
                pRIDs = oTemp.ToString();
                _SQL = string.Format("SELECT MIDS,FUNCTIONID FROM DM_ROLE WHERE RID IN({0})", pRIDs);
                dtRuleS = ORAHelper.ExecuteTable(_SQL);
            ...
            {
                strSQLControls = string.Format("SELECT NODEID,NODENAME,PARENTID,LV FROM DM_EXPRESS_NAVIGATION WHERE NODEID IN({0}) ORDER BY NODEID", pMIDs);
            }
        ...
        /// <returns></returns>
        public int ChildNumber(string pParentId)
        {
            _SQL = string.Format("SELECT COUNT(*) FROM DM_EXPRESS_NAVIGATION WHERE PARENTID = {0}", pParentId);
        ...
        /// <returns></returns>
        public DataTable GetLockObjectInfo(string pCategory)
        {
            _SQL = string.Format("SELECT FSTATUS,FUSER FROM DM_LOCKOBJECT WHERE CATEGORY = '{0}'", pCategory);
            return ORAHelper.ExecuteTable(_SQL);
        }
        ...
        /// <returns></returns>
        public object GetLockObjectInfo(string pCategory, int pStatus)
        {
            _SQL = string.Format("SELECT FUSER FROM DM_LOCKOBJECT WHERE CATEGORY = '{0}' AND FSTATUS = {1}", pCategory, pStatus);
        ...
        /// <param name="pCategory">类别</param>
        public void UpdateLockStatus(int pStatus, string pCategory)
        {
            if (pStatus == 1)//设置占用状态
            {
                _SQL = string.Format("UPDATE DM_LOCKOBJECT SET MODIFYDATE = SYSDATE, FSTATUS = '1', FUSER = '{0}' WHERE Category = '{1}'", GlobalParameter.K3Inf.UserName, pCategory);
            }
            else if (pStatus == 0)//解除占用状态
            {
                _SQL = string.Format("UPDATE DM_LOCKOBJECT SET MODIFYDATE = SYSDATE, FSTATUS = '0', FUSER = 'None' WHERE Category = '{0}'", pCategory);
            }
        ...
        /// <returns></returns>
        public DataTable DM_LockObjectInfo(string pCategory)
        {
            _SQL = string.Format("SELECT CASE WHEN CEIL((SYSDATE - MODIFYDATE) * 24 * 60) >= 5 THEN '0' ELSE FSTATUS END FSTATUS, FUSER,FTYPE FROM DM_LOCKOBJECT WHERE CATEGORY = '{0}'", pCategory);
            return ORAHelper.ExecuteTable(_SQL);
        }
        ...
        /// <param name="pCategory">分类</param>
        public void DM_LockObject_Add(string pObName, string pFType, string pDescription, string pCategory)
        {
            _SQL = string.Format("INSERT INTO DM_LOCKOBJECT(OBNAME,FUSER,FSTATUS,FTYPE,DESCRIPTION,CATEGORY) VALUES('{0}','None',0,'{1}','{2}','{3}')", pObName, pFType, pDescription, pCategory);
            ORAHelper.ExecuteNonQuery(_SQL);
        }
        ...
        /// <returns></returns>
        public int SolutionName(string pName)
        {
            _SQL = string.Format("SELECT COUNT(*) FROM DM_FILTER_SOLUTION WHERE SNAME = '{0}'", pName);
            return int.Parse(ORAHelper.ExecuteScalar(_SQL).ToString());
        }
        ...
        /// <returns></returns>
        public DataTable GetSolution(FormID pFormID)
        {
            if (GlobalParameter.K3Inf.UserName.ToUpper() == "DAMO")
                _SQL = string.Format("SELECT SNAME 方案名,CREATOR 创建人,TO_CHAR(CREATEDATE,'yyyy-mm-dd') 创建日期 FROM DM_FILTER_SOLUTION WHERE INSTR(FORMID,'{0}') > 0 ORDER BY SNAME", pFormID);
            else
                _SQL = string.Format("SELECT SNAME 方案名,CREATOR 创建人,TO_CHAR(CREATEDATE,'yyyy-mm-dd') 创建日期 FROM DM_FILTER_SOLUTION WHERE INSTR(FORMID,'{0}') > 0 AND (ISSHARE = '1' OR CREATOR = '{1}') ORDER BY SNAME", pFormID, GlobalParameter.K3Inf.UserName);
        ...
        /// <param name="pName">方案名</param>
        /// <returns></returns>
        public DataTable GetSolution(string pName)
        {
            _SQL = string.Format("SELECT SCONTENT,SROWS,CREATOR FROM DM_FILTER_SOLUTION WHERE SNAME = '{0}'", pName);
            return ORAHelper.ExecuteTable(_SQL);
        }
        ...
        public void SaveSolution(string pName, bool pIsShare, string pContent, int pRows, FormID pFromID)
        {
        ...
        /// <param name="pContent">内文</param>
        /// <param name="pRows">条件数</param>
        public void UpdateSolution(string pName, string pContent, int pRows)
        {
        ...
        /// </summary>
        /// <param name="pName">方案名</param>
        public void DelSolution(string pName)
        {
        ...
        /// <param name="pType">类型</param>
        /// <returns></returns>
        public DataTable NumberMatch(string pType)
        {
        ...
        /// <returns></returns>
        public bool NumberMatchExists(string pType, string pNumber)
        {
        ...
        /// <param name="pNumber">编码</param>
        /// <returns></returns>
        public bool NumberMatchExists(string pType, int pFID, string pNumber)
        {
        ...
        /// <returns></returns>
        public bool NumberMatchExists(string pType, string pNumber, int pLength, string pTypeName)
        {
        ...
        /// <param name="pTypeName">类型</param>
        /// <returns></returns>
        public bool NumberMatchExists(string pType, int pFID, string pNumber, int pLength, string pTypeName)
        {
        ...
        public void AddNumberMatch(string pNumber, int pLength, string pType, string pTypeNumber, string pTypeName, string pDescription, string pIsMatch)
        {
        ...
        /// <param name="pLength">编码长度</param>
        /// <param name="pTypeName">类型名称</param>
        public void UpdateNumberMatch(int pFID, string pNumber, int pLength, string pTypeName)
        {
        ...
        /// <param name="pDescription">描述</param>
        /// <param name="pIsMatch">是否匹配，默认'0'</param>
        public void UpdateNumberMatch(int pFID, string pNumber, string pDescription, string pIsMatch)
        {
        ...
        /// </summary>
        /// <param name="pFID">内码</param>
        public void DelNumberMatch(int pFID)
        {
        ...
        /// <param name="pUserOrgId">使用组织</param>
        /// <returns></returns>
        public DataTable CalculateStock(string pType, int pIndex, int pUserOrgId)
        {
            switch (pType)
            {
                case "LOCKSTOCK":
        ...
        /// <param name="pNumber">编码</param>
        /// <returns></returns>
        public bool CalculateNumberExists(string pType, int pSEQ, string pNumber)
        {
        ...
        public void AddCalculateStock(string pType, int pSEQ, int pStockId, string pStockNumber, string pStockName)
        {
        ...
        /// </summary>
        /// <param name="pFID">内码</param>
        public void UpdateCalculateStock(int pFID)
        {
        ...
        public void SaveCalculateStock(DataTable pDataTable)
        {
        ...
        /// <returns></returns>
        public DataTable Log_OrderLock(int pIndex, string pFBillNo, string pYSD, DateTime pFrom, DateTime pTo, string pFFLAG, string pMaterialiNo)
        {
            switch (pIndex)
            {
                case 1:
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable ExceptionRecord()
        {
            _SQL = @"SELECT '-1' FValue,'全部' FName FROM DUAL
            UNION
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable PickMTLDepartment()
        {
        ...
        /// <returns></returns>
        public bool PickMTLDeptExists(string pFNumber)
        {
        ...
        /// <param name="pUseOrgId">使用组织ID</param>
        public void AddPickMTLDept(string pDeptId, string pFNumber, string pFName, string pUseOrgId)
        {
        ...
        /// </summary>
        /// <param name="pFNumber">部门编码</param>
        public void DelPickMTLDept(string pFNumber)
        {
        ...
        /// <returns></returns>
        public DataTable BOMQuery(int pType, string pFilter)
        {
            switch (pType)
            {
                case 1:
        ...
        /// <param name="pFilter">过滤条件</param>
        /// <returns></returns>
        public DataTable MTL203(string pFilter)
        {
        ...
        /// <param name="pType">类型</param>
        /// <returns></returns>
        public DataTable Role(int pType)
        {
            if (pType == 0)
        ...
        public void DM_Log_Local(string pOName, string pONavi, string pOContent, string pFlag = "1")
        {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetDM_Dir_Stock()
        {
        ...
        public string SynMTLForWMS(bool pIsAll, string pFNumber)
        {
            string strReturn;
            if (pIsAll)
            {
                SQLHelper.ExecuteNonQuery("DM_P_SynMTL");
                strReturn = "同步物料执行完成。";
            }
            else
            {
                ...
                //LEFT JOIN T_BAS_ASSISTANTDATAENTRY_L AL16 ON A16.FENTRYID = AL16.FENTRYID AND AL16.FLOCALEID = 2052 
                //LEFT JOIN T_BAS_ASSISTANTDATAENTRY A17 ON M.F_ISBATCHMANAGER = A17.FENTRYID
                //LEFT JOIN T_BAS_ASSISTANTDATAENTRY_L AL17 ON A17.FENTRYID = AL17.FENTRYID AND AL17.FLOCALEID = 2052 
                ...
                //WHEN MATCHED THEN UPDATE SET FName = O.FNAME,fNumber = O.FNUMBER,oldNumber = O.OLDNUMBER,dataStatus = O.DATASTATUS,enabled = O.M_ENABLED,userOrgId = 100508,basicUnitId = O.BASICUNITID,materialSize = O.MATERIALSIZE,materialTypeid = O.MATERIALTYPEID
                ...
                //    ,middleSortId = O.MIDDLESORTID,middleSortName = O.MIDDLESORTNAME,middleSortNumber = O.MIDDLESORTNUMBER,smallSortId = O.SMALLSORTID, smallSortName = O.SMALLSORTNAME,smallSortNumber = O.SMALLSORTNUMBER,thinSortName = O.THINSORTID
                //    ,brandId = O.BRANDID,brandName = O.BRANDNAME,brandNumber = O.BRANDNUMBER,seriesId = O.SERIESID,seriesName = O.SERIESNAME,seriesNumber = O.SERIESNUMBER,productId = O.PRODUCTID,productName = O.PRODUCTNAME,productNumber = O.PRODUCTNUMBER
                //    ,carSeriesId = O.CARSERIESID,carSeriesName = O.CARSERIESNAME,carSeriesNumber = O.CARSERIESNUMBER,carTypeId = O.CARTYPEID,carTypeName = O.CARTYPENAME,carTypeNumber = O.CARTYPENUMBER,colorId = O.COLORID,colorName = O.COLORNAME,colorNumber = O.COLORNUMBER
                //    ,priceElementId = O.PRICEELEMENTID,priceElementName = O.PRICEELEMENTNAME,priceElementNumber = O.PRICEELEMENTNUMBER,technologyId = O.TECHNOLOGYID,technologyName = O.TECHNOLOGYNAME,technologyNumber = O.TECHNOLOGYNUMBER
                ...
                //WHEN NOT MATCHED THEN INSERT(FMATERIALID,FName,fNumber,oldNumber,dataStatus,enabled,userOrgId,basicUnitId,materialSize,materialTypeid,isBatchManager,isSnManager,isQualityPeriodManager
                //    ,batchRuleId,snRuleId,snUnitId,snManagerTypeId,qualityPeriodUnitId,qualityPeriod,safetyStock,lastUpdateDate,receiveMaxScale,receiveMinScale,bigSortId,bigSortName,bigSortNumber
                //    ,middleSortId,middleSortName,middleSortNumber,smallSortId, smallSortName,smallSortNumber,thinSortName,brandId,brandName,brandNumber,seriesId,seriesName,seriesNumber
                //    ,productId,productName,productNumber,carSeriesId,carSeriesName,carSeriesNumber,carTypeId,carTypeName,carTypeNumber,colorId,colorName,colorNumber,priceElementId,priceElementName,priceElementNumber
                //    ,technologyId,technologyName,technologyNumber,structureId,structureName,structureNumber,categoryId,categoryName,categoryNumber,linesId,linesName,linesNumber,finishReceiptOverRate,finishReceiptShortRate,isAotuBringOut,mtlplace
                //)VALUES(O.FMATERIALID,O.FNAME,O.FNUMBER,O.OLDNUMBER,O.DATASTATUS,O.M_ENABLED,100508,O.BASICUNITID,O.MATERIALSIZE,O.MATERIALTYPEID,O.ISBATCHMANAGER,O.ISSNMANAGER,O.ISQUALITYPERIODMANAGER
                //    ,O.BATCHRULEID,O.SNRULEID,O.SNUNITID,O.SNMANAGERTYPEID,O.QUALITYPERIODUNIT,O.QUALITYPERIOD,O.SAFETYSTOCK,O.LASTUPDATEDATE,O.RECEIVEMAXSCALE,O.RECEIVEMINSCALE,O.BIGSORTID,O.BIGSORTNAME,O.BIGSORTNUMBER
                //    ,O.MIDDLESORTID,O.MIDDLESORTNAME,O.MIDDLESORTNUMBER,O.SMALLSORTID,O.SMALLSORTNAME,O.SMALLSORTNUMBER,O.THINSORTID,O.BRANDID,O.BRANDNAME,O.BRANDNUMBER,O.SERIESID,O.SERIESNAME,O.SERIESNUMBER
                //    ,O.PRODUCTID,O.PRODUCTNAME,O.PRODUCTNUMBER,O.CARSERIESID,O.CARSERIESNAME,O.CARSERIESNUMBER,O.CARTYPEID,O.CARTYPENAME,O.CARTYPENUMBER,O.COLORID,O.COLORNAME,O.COLORNUMBER
                //    ,O.PRICEELEMENTID,O.PRICEELEMENTNAME,O.PRICEELEMENTNUMBER,O.TECHNOLOGYID,O.TECHNOLOGYNAME,O.TECHNOLOGYNUMBER,O.STRUCTUREID,O.STRUCTURENAME,O.STRUCTURENUMBER,O.CATEGORYID,O.CATEGORYNAME,O.CATEGORYNUMBER
                //    ,O.LINESID,O.LINESNAME,O.LINESNUMBER,O.FINISHRECEIPTOVERRATE,O.FINISHRECEIPTSHORTRATE,O.F_PAEZ_AUTOCATCH,O.F_PAEZ_Place
                //);";
                ...
                string strExec = SQLHelper.ExecuteScalar(CommandType.StoredProcedure, "DM_P_SynMTLByNumber", parms).ToString();

                SQLHelper.ExecuteScalar(strExec);
                strReturn = "物料[" + pFNumber + "]已经同步";
            }
        ...
        public void UpdateMStockSetting(string pFID, string pFName, string pFNameTran)
        {
        ...
        public void DeleteMStockSetting(List<string> pFIDs)
        {
