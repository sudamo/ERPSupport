...
namespace ERPSupport.SQL.K3Cloud
{
    ...
    public static class CommonFunction
    {
        ...
        public static void UpdateLockStatus(int pStatus, string pCategory)
        {
            if (pStatus == 1)//设置占用状态
            {
                _SQL = "UPDATE DM_LOCKOBJECT SET MODIFYDATE = SYSDATE, FSTATUS = '1', FUSER = '" + Model.Globa.GlobalParameter.K3Inf.UserName + "' WHERE Category = '" + pCategory + "'";
            }
        ...
        {
            _SQL = @"SELECT FID 内码, A.FMATERIALNUMBER 物料编码, B.FNAME 物料名称, C.FNAME 部门, A.FSTOCKNUMBER 仓库编码, D.FNAME 仓库,NVL(A.FCREATOR,' ') 创建人, A.FCREATEDATE 创建日期
        ...
        public static void UpdateMStockSetting(string pStockNumber, int pFID)
        {
            ...
            _SQL = "UPDATE T_AUTO_MSTOCKSETTING SET FSTOCKID = " + iStockId + ", FSTOCKNUMBER = '" + pStockNumber + "',FCREATOR = '" + Model.Globa.GlobalParameter.K3Inf.UserName + "',FCREATEDATE = SYSDATE WHERE FID = " + pFID;
