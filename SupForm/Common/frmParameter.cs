        ...
        private void ImportReceivable()
        {
            ...
            for (int i = 3; i <= worksheet.UsedRange.Rows.Count; i++)
            {
                try
                {
                    ...
                    FNumber = CommonFunction.GetMTLIDByNumber(100508, FNumber.ToString());
                    ...
                    CommonFunction.UpdateMTLPara(new MaterialParameter(int.Parse(F_PAEZ_SAFEDAYS), int.Parse(F_PAEZ_LOGISTICSDAYS), int.Parse(F_PAEZ_REPLENISHMENT), decimal.Parse(F_PAEZ_LOWQTY), decimal.Parse(F_PAEZ_MINQTY)), int.Parse(FNumber.ToString()));
                }
                catch (Exception ex)
                {
